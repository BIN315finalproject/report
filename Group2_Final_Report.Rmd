...
This study investigated RNA-seq data to gain insights into colorectal cancer (CRC). The raw dataset contained 60,616 genes and 522 samples, including 481 primary tumor samples and 41 normal tissue samples. Comprehensive preprocessing steps were performed, including removal of redundant transcripts, filtering of low-quality samples and genes, identification and removal of outliers by PCA, and normalization of the data. PCA revealed clear distinctions between tumor and normal samples based on linear combinations of genes. Differential expression analysis identified 1,389 upregulated genes and 1,414 downregulated genes.

For classification of tumor and normal samples, we used two supervised machine learning algorithms, K-nearest neighbors (KNN) and random forest (RF), achieving 100% accuracy without any signs of overfitting and data leakage. Before building the model, the weights of the samples need to be calculated to address the imbalance of the number of instances between classes and used as parameters when fitting the dataset. The random forest model identified important genes that significantly contributed to the classification. GO enrichment analysis of these genes and DEGs highlighted key biological pathways and processes, such as cell signaling (particularly the Wnt pathway) and extracellular matrix organization, which are essential for tumorigenesis. These findings confirm the direction of our study and provide further biological insights into CRC.
...
PCA is an unsupervised dimensionality reduction method that transforms the original feature set into a set of linearly independent variables. This transformation can help us not only maintain most of the original information when selecting several principle components, but also improve computational efficiency and dataset visualization and feature selection [3]. Here, we performed principal component analysis by the prcomp() function to better understand our dataset and choose the number of principal components for dimensionality reduction.
...
We used the DESeq2 package to perform the variance-stabilizing transformation (VST) on the count data.
...
#### 3.2.2 Visualization on the First Two Principal Components
...
#### 3.2.3 Outlier Detection through PCA
...
The model has an accuracy of 100%. It may indicate overfitting or Data leakeage.
...
Gene Ontology (GO) enrichment analysis highlighted key biological processes associated with the top genes that distinguished tumor and normal samples. From the differential expression analysis, we identified significantly up-regulated and down-regulated genes, as well as significant genes identified by random forest models. GO enrichment analysis allowed us to link these genes to relevant biological processes.
...
#### 3.5.2 GO Analysis of genes seleted from rf model
