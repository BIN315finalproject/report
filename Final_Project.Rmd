...
Therefore, we tried to check the potential outliers found before and remove them, because the number of these outliers was very small, only 18 samples, and these outliers could have a great impact on the subsequent analysis, so we decided to remove them and perform the PCA analysis again to check the results of the before and after PCA.
```{r PCA5}
...
# go_pc1 <- enrichGO(gene = genes_pc1, OrgDb = org.Hs.eg.db, keyType = "ENSEMBL", ont = "BP")
# go_pc1_df <- as.data.frame(go_pc1)
# go_pc2 <- enrichGO(gene = genes_pc2, OrgDb = org.Hs.eg.db, keyType = "ENSEMBL", ont = "BP", pvalueCutoff = 0.1)
# go_pc2_df <- as.data.frame(go_pc2)
